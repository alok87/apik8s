...
type MykindLister interface {
	...
	List(selector labels.Selector) (ret []*group.Mykind, err error)
...
// List lists all Mykinds in the indexer.
func (s *mykindLister) List(selector labels.Selector) (ret []*group.Mykind, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*group.Mykind))
	})
	return ret, err
}
...
type MykindNamespaceLister interface {
	...
	Get(name string) (*group.Mykind, error)
...
// List lists all Mykinds in the indexer for a given namespace.
func (s mykindNamespaceLister) List(selector labels.Selector) (ret []*group.Mykind, err error) {
...
// Get retrieves the Mykind from the indexer for a given namespace and name.
func (s mykindNamespaceLister) Get(name string) (*group.Mykind, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(group.Resource("mykind"), name)
	}
	return obj.(*group.Mykind), nil
}
